...
function adjustModalProperties(){var dim=getDim();if(contentType!=="error"){setWindowMargin(dim[0]+(oB.settings.contentBorderWidth*2),dim[1]);}else{dim[1]=250;}
...
$('#ob_title').append(title).click(function(e){e.stopPropagation();});if(oB.settings.addThis&&obj.data('oB').share!=="false"){addThis.addClass("ob_share-"+title.replace(/ /g,"_"));if(title===""){title="share";}
...
setControls();if(obj.data('oB').share!=="false"){$(document).trigger('oB_init',ob_link);oB.methods.logit('ID:'+oB.currentIndex+' href:"'+href+'" link:"'+ob_link+'"',true);}else{$(document).trigger('oB_init',"");oB.methods.logit('ID:'+oB.currentIndex+' href:"'+href+'"',true);}
...
obj.data('oB').css=[dim[0],dim[1]];content=$('<iframe allowTransparency="true" id="ob_video" height="100%" width="100%" type="text/html" frameborder="0" hspace="0" scrolling="auto" src="'+src+'"></iframe>').css({"height":dim[0],"width":dim[1],"background-color":"#000000"});buildWindow();}
function showFlash(){var dim=oB.methods.getSize(obj,[0,0]);obj.data('oB').css=[dim[0],dim[1]];content=$('<div id="ob_video"><embed flashVars="playerVars=autoPlay=yes" src="'+href+'" wmode="transparent" pluginspage="http://www.macromedia.com/go/getflashplayer" allowFullScreen="true" allowScriptAccess="always" width="'+dim[1]+'" height="'+dim[0]+'" type="application/x-shockwave-flash"></embed></div>').css({"height":dim[0],"width":dim[1]});buildWindow();}
