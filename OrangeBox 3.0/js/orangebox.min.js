...
if(u.match(/(\?|\&)(iframe\=true)((\?|\&)(width\=\d+(\&height\=\d+)?|height\=\d+(\&width\=\d+)?))?$/)){c="iframe";}else if(u.match(/\.(?:jpg|jpeg|bmp|png|gif)((\?|\&)(width\=\d+(\&height\=\d+)?|height\=\d+(\&width\=\d+)?))?$/)){c="image";if(o.parent().hasClass('ngg-gallery-thumbnail')&&oB.settings.nextGen){child=o.children('img')[0];if(typeof o.attr('title')!=="undefined"){cap=o.attr('title');}
if(typeof child.attr('alt')!=="undefined"){t=child.attr('alt');}}}else if(u.match(/\.pdf((\?|\&)(width\=\d+(\&height\=\d+)?|height\=\d+(\&width\=\d+)?))?$/)){c="pdf";}else if(u.match(/\.swf((\?|\&)(width\=\d+(\&height\=\d+)?|height\=\d+(\&width\=\d+)?))?$/)){c="flash";}else if(u.match(/^http:\/\/api\.flickr\.com\/services\/feeds\/.{1,}\.gne\?id\=\d{1,}\@.{1,}\&lang\=.{1,}\&format\=rss\_200/)){c="flickr";u=u.replace('rss_200','json')+"&jsoncallback=?";if(!rel.match(/\[/)){g='flickr'+newDate.getTime();}
...
dim=[Math.round(size[0]),Math.round(size[1])];if(contentType!=="error"){setWindowMargin(dim[0]+(oB.settings.contentBorderWidth*2),dim[1]);}else{dim[1]=250;}
...
function showiFrame(){var dim=oB.methods.getSize(obj,[0,0]),newhref;newhref=(contentType==="pdf")?"http://docs.google.com/viewer?url="+encodeURIComponent(href)+"&embedded=true&iframe=true":href;newhref=newhref.replace(/(\?|\&)iframe\=true/,'');newhref=newhref.replace(/(\?|\&)width\=\d{1,}/,'');newhref=newhref.replace(/(\?|\&)height\=\d{1,}/,'');obj.data('oB').css=dim;content=$('<div id="ob_iframe"><iframe allowTransparency="true" height="100%" width="100%" scrolling="auto" type="text/html" frameborder="0" hspace="0" src="'+newhref+'"></iframe></div>').css("width",dim[1]);if(dim[0]!==0){content.css("height",dim[0]);}
buildWindow();}
...
obj.data('oB').css=dim;content=$('<div id="ob_iframe"><iframe allowTransparency="true" id="ob_video" width="100%" height="100%" type="text/html" frameborder="0" hspace="0" scrolling="auto" src="'+src+'"></iframe></div>').css({"height":dim[0],"width":dim[1],"background-color":"#000000"});buildWindow();}
...
if(content.href==="undefined"||content.href==="")return false;var obj=$('<a href="'+content.href+'"></a>'),html;if(content.title)obj.attr('title',content.title);if(content.caption)obj.attr('data-ob_caption',content.caption);if(content.link)obj.attr('data-ob_link',content.link);if(content.linkText)obj.attr('data-ob_linkText',content.linkText);if(content.linkTarget)obj.attr('data-ob_linkTarget',content.linkTarget);if(content.delay)obj.attr('data-ob_delayTimer',content.delay);if(content.share)obj.attr('data-ob_share','true');else obj.attr('data-ob_share','false');if(content.html){html=$(content.html).css('display','none');$(body).append(html);}
if(content.gallery)obj.attr('rel',oB.settings.searchTerm+'['+content.gallery+']');else obj.attr('rel',oB.settings.searchTerm);oB.methods.setupData(obj);oB.methods.create(obj);},navigate:function(d,i,o){if(o){$.extend(oB.settings,o);}
if(!i){if(d===1){i=oB.currentIndex+1;}else if(d===-1){i=oB.currentIndex-1;}}
if(oB.currentGallery[i]){oB.progress=true;$(document).trigger('oB_navigate',[i]);$('#ob_content').fadeOut(oB.settings.fadeTime,function(){if($('#ob_inline').length){$('#ob_inline').find('.ob_inline_content').appendTo('.ob_inline_content_holder').removeClass('ob_inline_content');$('.ob_inline_hide').hide().removeClass('ob_inline_hide');$('.ob_inline_content_holder').removeClass('ob_inline_content_holder');}
