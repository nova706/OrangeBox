...
return false;},setupData:function(o){var u=o.attr('href'),c=false,s=[0,0],i=0,t="",g=false,rel=o.attr('data-ob'),id,alias=false,unique=true,cap=o.attr('data-ob_caption'),child;if(!rel||rel===""){rel=o.attr('rel');o.attr('data-ob',rel);}
...
if(typeof o.attr('data-ob_iframe')!=='undefined'&&o.attr('data-ob_iframe')==="true"){c="iframe";}else if(u.match(/\.(?:jpg|jpeg|bmp|png|gif)/)){c="image";if(o.parent().hasClass('ngg-gallery-thumbnail')&&oB.settings.nextGen){child=o.children('img')[0];if(typeof o.attr('title')!=="undefined"){cap=o.attr('title');}
if(typeof child.attr('alt')!=="undefined"){t=child.attr('alt');}}}else if(u.match(/\.pdf/)){c="pdf";}else if(u.match(/\.swf/)){c="flash";}else if(u.match(/^http:\/\/api\.flickr\.com\/services\/feeds\/.{1,}\.gne\?id\=\d{1,}\@.{1,}\&lang\=.{1,}\&format\=rss\_200/)){c="flickr";u=u.replace('rss_200','json')+"&jsoncallback=?";if(!rel.match(/\[/)){g='flickr'+newDate.getTime();}
$.getJSON(u,function(data){$.each(data.items,function(index,item){var item_href=item.media.m.replace('_m.jpg','.jpg');if(index===0){o.attr({href:item_href,"data-ob_share":"false",caption:cap,title:item.title});oB.methods.setupData(o);}else if(index<oB.settings.streamItems){oB.methods.setupData($('<a href="'+item_href+'" data-ob_linkText="'+o.attr('data-ob_linkText')+'" data-ob_link="'+o.attr('data-ob_link')+'" data-ob_linkTarget="'+o.attr('data-ob_linkTarget')+'" data-ob_delayTimer="'+o.attr('data-ob_delayTimer')+'" data-ob_share="false" title="'+item.title+'" data-ob="lightbox['+g+']"></a>'));}else{return false;}});});}else if(u.match(/^https:\/\/picasaweb\.google\.com\/data\/feed\/base\//)){c="picasa";u=u.replace('/base/','/api/').replace('alt=rss','alt=json-in-script')+'&max-results='+oB.settings.streamItems+'&callback=?';if(!rel.match(/\[/)){g='picasa'+newDate.getTime();}
...
if(index===0){o.attr({href:picasaSrc,"data-ob_share":"false","data-ob_caption":cap,title:item.title.$t});oB.methods.setupData(o);}else if(index<oB.settings.streamItems){oB.methods.setupData($('<a href="'+picasaSrc+'" data-ob_caption="'+item.summary.$t+'" data-ob_linkText="'+o.attr('data-ob_linkText')+'" data-ob_link="'+o.attr('data-ob_link')+'" data-ob_linkTarget="'+o.attr('data-ob_linkTarget')+'" data-ob_delayTimer="'+o.attr('data-ob_delayTimer')+'" data-ob_share="false" title="'+item.title.$t+'" data-ob="lightbox['+g+']"></a>'));}else{return false;}});}});}else if(u.match(/^http:\/\/\w{0,3}\.?youtube\.\w{2,3}\/watch\?v=[\w\-]{11}/)){id=u.match(/\?v=[\w\-]{11}/)[0].substring(3);c="youtube";}else if(u.match(/^http:\/\/\w{0,3}\.?youtu\.be\/[\w\-]{11}$/)){id=u.match(/youtu\.be\/[\w\-]{11}$/)[0].substring(9);c="youtube";}else if(u.match(/^http:\/\/\w{0,3}\.?youtube\.\w{2,3}\/embed\/[\w\-]{11}$/)){id=u.match(/\/embed\/[\w\-]{11}$/)[0].substring(7);c="youtube";}else if(u.match(/^http:\/\/\w{0,3}\.?vimeo\.com\/\d{1,10}$/)){id=u.match(/vimeo\.com\/\d{1,}/)[0].substring(10);c="vimeo";}else if(u.match(/^http:\/\/\w{0,3}\.?viddler\.com\/(?:simple|player)\/\w{1,10}$/)){id=u.match(/viddler\.com\/(player|simple)\/\w{1,}/)[0].substring(19);c="viddler";}else if(u.match(/^#\w{1,}$/)){c="inline";}else{oB.methods.logit('Unsupported Media: '+u);}
...
function showiFrame(){var dim=oB.methods.getSize(obj,[0,0]),newhref;newhref=(contentType==="pdf")?"http://docs.google.com/viewer?url="+encodeURIComponent(href)+"&embedded=true":href;obj.data('oB').css=dim;content=$('<div id="ob_iframe"><iframe allowTransparency="true" height="100%" width="100%" scrolling="auto" type="text/html" frameborder="0" hspace="0" src="'+newhref+'"></iframe></div>').css("width",dim[1]);if(dim[0]!==0){content.css("height",dim[0]);}
buildWindow();}
...
jQuery(document).ready(function($){var searchTerm='a[data-ob*=lightbox], area[data-ob*=lightbox], a[rel*=lightbox], area[rel*=lightbox]';if(oB.settings.searchTerm!==""){searchTerm='a[data-ob*='+oB.settings.searchTerm+'], area[data-ob*='+oB.settings.searchTerm+'], a[rel*='+oB.settings.searchTerm+'], area[rel*='+oB.settings.searchTerm+']';}
